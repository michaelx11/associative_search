...
fn find_associations(search_set: &[&str], norm_index: &indexer::FstIndex, table_index: &indexer::FstIndex) -> HashMap<String, HashMap<String, String>> {
...
fn process_query(query: &str, norm_index: &indexer::FstIndex, table_index: &indexer::FstIndex) -> String {
    // Get query set, split by ","
    let search_terms: Vec<&str> = query.split(",").collect();
    let mut association_dict = find_associations(&search_terms[..], norm_index, table_index);
    // Finally, we check if we got any good associations
    let mut association_count_dict: HashMap<String, usize> = HashMap::new();
    for item in search_terms.iter() {
...
fn main() {
    ...
    while true {
        ...
        let results = process_query(&line, &norm_index, &table_index);
